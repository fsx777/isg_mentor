    ...
    if latest_question:
        with st.expander("🧠 Otonom Botun Günlük Ters Köşe Sorusu", expanded=True):
            ...
            if st.button("Cevapla ve Değerlendir", key=f"btn_{latest_question['id']}"):
                if kullanici_cevabi:
                    secenek_index = secenekler.index(kullanici_cevabi)
                    kullanici_harfi = ['A', 'B', 'C', 'D', 'E'][secenek_index]
                    dogru_cevap_harfi = str(latest_question.get('dogru_cevap', '')).strip().upper()
                    if kullanici_harfi == dogru_cevap_harfi:
